...
def transfer_s3_to_rds(*args, **kwargs):
    ...
    rds_endpoint = "de-3-1-mysql.ch4xfyi6stod.ap-northeast-2.rds.amazonaws.com"
    
  
    rds_username = "admin"
    rds_password = "dethreeadmin3"

 
    rds_database = "de-3-1-mysql"

    engine = create_engine(f"mysql+pymysql://{rds_username}:{rds_password}@{rds_endpoint}/{rds_database}")

    data.to_sql('live_weather', engine, if_exists='replace', index=False) #if_exists='append'
